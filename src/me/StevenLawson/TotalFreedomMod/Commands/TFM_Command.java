...
public abstract class TFM_Command
{
    ...
    public boolean senderHasPermission()
    {
        ...
        if (level == AdminLevel.SUPER && !isSuper)
        {
            ...
            if (level == AdminLevel.SUPPORTER
                        && !EFMR_DonatorList.isSuperDonor(commandSender))
                {
                    return false;
                }
                if (level == AdminLevel.SUPPORTERPLUS
                        && !EFMR_DonatorList.isSeniorDonor(commandSender))
                {
                    return false;
                }
                if (level == AdminLevel.ARCHITECT
                        && !EFMR_BuilderList.isSuperBuilder(commandSender))
                {
                    return false;
                }
